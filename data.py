...
def insert_program(program_data):
    conn, cur = connect_to_db(db_path)
    query = 'INSERT INTO programs (program_type, name, salary, duration, description, url) VALUES (?,?,?,?,?,?)'
    values = (program_data['program_type'], program_data['name'],
...
def update_program(program_data):
    conn, cur = connect_to_db(db_path)
    query = "UPDATE programs SET program_type=?, name=?, salary=?, duration=?, description=?, url=? WHERE id=?"
    values = (program_data['program_type'], program_data['name'],
              program_data['salary'], program_data['duration'],
              program_data['description'], program_data['url'],
